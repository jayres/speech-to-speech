...
AUDIO_PROMPT_PATH = "./voices/espanol_mujer_0.wav"
DEFAULT_OLLAMA_MODEL = "llama3.1:latest"
# base.en is the default model for Whisper but only supports English. You can 
# change this to `base` to use the default model which supports multiple languages 
# but will be slower.
DEFAULT_WHISPER_MODEL = "base"

console = Console()
stt = whisper.load_model(DEFAULT_WHISPER_MODEL)


# Parse command line arguments
parser = argparse.ArgumentParser(description="Local Voice Assistant with ChatterBox TTS")
parser.add_argument("--voice", type=str, default=AUDIO_PROMPT_PATH, help="Path to voice sample for cloning")
parser.add_argument("--exaggeration", type=float, default=0.8, help="Emotion exaggeration (0.0-1.0)")
parser.add_argument("--cfg-weight", type=float, default=0.5, help="CFG weight for pacing (0.0-1.0)")
parser.add_argument("--model", type=str, default=DEFAULT_OLLAMA_MODEL, help="Ollama model to use")
...
def transcribe(audio_np: np.ndarray) -> str:
    ...
    if isinstance(text_field, list):
        text = " ".join(str(x) for x in text_field).strip()
    else:
...
def get_llm_response(text: str) -> str:
    ...
    # Print the LLM response before any further processing
    console.print(f"[magenta]LLM raw response:")
    console.print(f"[cyan]{response.strip()}")
...
if __name__ == "__main__":
    ...
    console.print(f"[yellow]Emotion exaggeration: {args.exaggeration}")
    console.print(f"[yellow]CFG weight: {args.cfg_weight}")
    console.print(f"[yellow]LLM model: {args.model}")
    ...
    try:
        while True:
            ...
            if audio_np.size > 0:
                ...
                console.print(f"[blue]You: {text}")

                with console.status("[yellow]Generating response...", spinner="dots"):
